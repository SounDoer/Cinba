...
- [ ] `startCore` 的 stdio 把 stderr 从 `"inherit"` 改成 `"pipe"`，由调用方转发。
      现状是 Electron 下 Pi 的报错完全不可见，下一次 Pi 出问题会同样难查。
      改动会波及 `scripts/repl.ts`（需自行把 stderr 打到终端）。
