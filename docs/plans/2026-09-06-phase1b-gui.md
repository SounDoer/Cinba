...
**6. 权限门原本挂在调用方，事后收归 `core-host`。** `main.ts` 与 `repl.ts` 各自解析
`@cinba/extensions/src/permission-gate.ts` 再通过 `startCore({ extensions: [gate] })` 传进去，
有三个问题：`desktop` 摸到了设计文档第 5 节禁止它依赖的 `extensions`；这个依赖压根没在
`desktop/package.json` 里声明（能跑只因 npm workspaces 把本地包都提升到了根 node_modules）；
同一段代码写了两遍，阶段 2 还要写第三遍。

收尾时已改为 `core-host` 自己解析并**无条件**挂上权限门，调用方传不了也漏不掉。理由不只是整洁：
权限门是这个 agent 的固有属性，不是某个界面的功能——交给调用方就意味着「谁忘了传谁裸奔」。
已加测试锁住该性质。
...
- [~] **Step 6: 若失败，按序尝试备用方案** —— 未触发（Step 5 一次通过）
...
（收尾时实际为 33 个：执行期修 bug 又补了 core-host 的 3 个与用户消息的 2 个。完整命令：`node --test "packages/core-host/src/*.test.ts" "packages/core-client/src/*.test.ts"`）
...
- [x] **Step 2: 在 renderer.js 里加工具卡片渲染**
...
- [x] **Step 3: 把新动作接进 applyAction 与 renderSnapshot**
...
- [x] **Step 3: 改掉 text_appended 分支（节点结构变了）**
...
- [~] **Step 3: 若前两步需要改代码，改完后提交** —— 未触发（两步均无需改动）
...
期望：33 个全部 pass（core-host 3 + core-client 30）。
...
- [x] `node --test "packages/core-host/src/*.test.ts" "packages/core-client/src/*.test.ts"` 33 个全绿
- [x] Task 9 Step 2 的八项手工验收全部通过
- [x] 渲染层没有引入任何打包器或框架
- [x] 渲染层不 import 任何本地包，不碰 Node
