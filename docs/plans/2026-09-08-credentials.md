...
**2. ❌ ~~原本担心的坑不存在：运行中的 Pi 立刻就能看到新 provider。~~
这条结论是错的，2026-09-09 由 `762ac61` 推翻。**
...
**但它只在那个探针的条件下成立**：那个 Pi 启动时一个凭据都没有。真实用法里
**Pi 只在启动时读一次凭据文件**，之后全靠内存里的快照，所以后加的 key 对它不可见——
模型列表里没有那家，`set_model` 报 "Model not found"。而凭据是核心自己进程里的
`ModelRuntime` 写的，所以 provider 面板说"已配置"、模型选择器却看不到，
**两者在问两个不同的进程**。

修法见 `762ac61`：凭据一变就给受影响的对话换一个 Pi——没人看的只停掉（下次打开自然从
磁盘回来），有人看的立刻重开。

**教训**：探针给的是"在这个条件下会怎样"，不是"总是会怎样"。这条探针的条件
（全新的空凭据目录）恰好是真实用法里最不常见的一种，而我把它当成了普遍结论。
