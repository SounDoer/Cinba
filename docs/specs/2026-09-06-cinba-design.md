...
**外出时不连家里，只连 VPS。** 「在外面用家里的项目」会把当初暂缓形态 A 的那个理由原样
带回来（家里电脑得一直醒着），而且要求家里的核心也开门。用户的选择是：真要在外面干的项目，
在 VPS 上也 clone 一份仓库。代价是 git 只同步提交过的东西，出门前得 push——这是习惯，
不是功能。

**这个选择带来一个重要的简化：只有 VPS 那个核心需要开门，家里的核心永远只听
