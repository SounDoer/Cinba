...
日期：2026-09-06（最后更新 2026-09-09）
状态（2026-09-09）：阶段 0 / 1a / 1b / 2 / 3a / 3b-1 已完成。此后按形态 C 推进，
已完成模型切换、多会话、TUI 客户端化、凭据管理、核心身份（形态 C 的第 1 和 2a 步）。
下一步是第 3 步（VPS + Tailscale）。多用户已于 2026-09-09 讨论完毕——不做，一人一份，
隔离交给操作系统，见第 2 节。
...
**Tailscale 认的是设备，不是人。** 两份 Cinba 监听在同一台 VPS 的 Tailscale 地址上，
家人手机只要在同一个 tailnet 里，就能连上你那一份——而连上就是完整 shell。

系统账号隔离了**数据**，没隔离**访问**。解法仍然不是我们发 token，而是继续往下推一层：

| 办法 | 谁来认证 |
|---|---|
| Tailscale ACL（按用户/设备限制可达端口） | Tailscale 自己 |
| 只监听 loopback + SSH 隧道 | SSH 密钥 |
| ~~自己发 token~~ | 明确不做 |

**对代码的唯一影响**：第 3 步可能需要 `CINBA_HOST`（监听地址可配），这样才**有得选**——
开到 tailnet，还是只听 loopback 走隧道。它必须受第 9 节红线约束：允许 loopback 与私有
网络地址，拒绝 `0.0.0.0` 与公网地址。这是第 3 步要设计的东西，现在不做。
...
存在每个前端里，就是三份各自的状态。**哪个对取决于实际怎么切**，而现在还没有第二个
核心可切，做了也验证不了。等 VPS 真跑起来再定。
...
| 事情 | 触发信号 | 理由记在 |
|---|---|---|
| **核心切换器**（界面内置核心清单） | VPS 真跑起来，且摸清了自己实际怎么切（在手机上切，还是在桌面上切） | 第 7 节形态 C，2b |
...
**留下的缺口，到期日是形态 C 的第 3 步**：Tailscale 认的是设备不是人，所以系统账号
隔离了数据、没隔离访问。解法在下一层（Tailscale ACL，或只听 loopback 走 SSH 隧道），
可能需要给核心加 `CINBA_HOST`。详见第 2 节。
