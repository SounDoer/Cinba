...
下一步是第 3 步（VPS + Tailscale），**但它之前必须先想清楚多用户**——见第 8 节的暂缓清单 B。
...
**多用户。** 用户 2026-09-08 提到家人也会用 VPS 上的 Cinba。这**打破了第 2 节明确写下的
非目标「不做多用户、鉴权、团队协作。单人自用。」**，影响远不止内存：

- 会话列表现在对所有客户端一视同仁——家人会看到你所有对话，你也会看到他们的
- 权限确认是广播的，**谁先答谁算数**：家人可以替你批准一个跑在 VPS 上的命令
- 花的是同一把 key、同一笔钱，用量统计混在一起
- 工作目录随便切，谁都能把 agent 指向 VPS 上任何目录

**到期日是形态 C 的第 3 步**：门一开，「你自己」就变成「任何连上来的人」。
所以这件事必须在部署到 VPS 之前想清楚，不能边做边说。

已经提前埋下的唯一一件事：**凭据操作只接受 loopback 连接**（第 9 节），
所以至少家人改不了你的 key。
