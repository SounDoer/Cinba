...
```
1. 多会话 + TUI/desktop 都改成 core-server 的客户端
2. 多核心：界面上能选连哪个 core-server
3. VPS 上部署 + Tailscale + 红线加例外
4. 手机（给 web 加响应式）
```
...
形态 C 落地时，有些本机成立的能力必须按连接来源关掉，因为「你自己」在远程场景里
变成了「任何连上来的人」：
