...
状态（2026-09-16）：阶段 0 / 1a / 1b / 2 / 3a / 3b-1 与形态 C 第 1、2a、2b、3 步的代码均已完成；
2b 的 Windows 视觉验收与 macOS 双 Core 实机验收待完成。
...
2. 多核心
   ...
   2b. 🧪 Desktop Core Navigator：App 保存核心清单       （代码完成，待双平台实机验收）
...
**2b 于 2026-09-15 重新启动**：VPS 与 Home Mac 两个真实 Core 让 Desktop 的多 Core 入口从预想
变成了日常场景。结论不是把清单塞进 Web UI，而是让独立于任一 Core 的 Desktop 保存 Profiles：
Web UI 继续管理“这个 Core 里面有什么”，Desktop 管理“我要进入哪个 Core”。浏览器仍用书签，
TUI 仍用 `CINBA_SERVER`；Desktop 加载目标 Core 自己提供、与其版本匹配的完整页面。

这样解决了原先的鸡生蛋问题：即使某个 Core 离线，Desktop shell 和清单仍然存在；也避免不同
origin 的 `localStorage` 各存一份互不相通的配置。详细设计见
`docs/specs/2026-09-15-desktop-multi-core-design.md`，实施计划见
`docs/plans/2026-09-15-desktop-multi-core.md`。
