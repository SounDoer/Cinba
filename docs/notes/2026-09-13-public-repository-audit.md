...
日期：2026-09-13
状态：代码内容未发现阻塞项；Git 提交身份是否公开待用户确认
...
- 接受公开：无需处理；
- 不接受公开：必须在切换仓库可见性前重写全部 commit 的身份并 force push。

历史重写会改变所有 commit hash，并影响现有 clone、将来的 `prod` 分支和文档中引用的 hash，
因此不得自动执行，必须由用户明确决定。
...
仓库内容可以进入公开流程；唯一待确认项是 commit 身份是否可以公开。
