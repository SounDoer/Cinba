...
test("the stable launcher follows current and preserves user arguments", async (t) => {
  const root = temporaryDirectory("cinba-stable-launcher-", t);
...
test("the stable launcher fails closed when current is missing or inconsistent", async (t) => {
  const root = temporaryDirectory("cinba-stable-launcher-invalid-", t);
  const paths = layout(root);
  const target = requireProductTarget();
