...
test("handoff uses a fixed absolute state path and an atomic private file", async (t) => {
  const root = temporaryDirectory("cinba-update-handoff-", t);
  const handoff = desktopHandoff(root);
...
test("handoff storage refuses a symbolic-link directory", async (t) => {
  const root = temporaryDirectory("cinba-update-handoff-link-", t);
...
test("claimed handoff must match every field of the current ready candidate", async (t) => {
  const root = temporaryDirectory("cinba-update-handoff-ready-", t);
  const handoff = desktopHandoff(root);
...
test("a crashed helper's expired handoff is reclaimed while the new lease is held", async (t) => {
  const root = temporaryDirectory("cinba-update-handoff-stale-", t);
...
test("an unexpired handoff is not replaced by a new lease", async (t) => {
  const root = temporaryDirectory("cinba-update-handoff-active-", t);
...
test("a malformed ordinary handoff can be recovered only while a different lease is held", async (t) => {
  const root = temporaryDirectory("cinba-update-handoff-malformed-", t);
...
test("stale recovery unlinks a handoff directory symlink without following it", async (t) => {
  const root = temporaryDirectory("cinba-update-handoff-recover-link-", t);
  const outside = temporaryDirectory("cinba-update-handoff-outside-", t);
...
test("an old worker cannot delete a newer handoff id", async (t) => {
  const root = temporaryDirectory("cinba-update-handoff-id-", t);
...
test("conditional handoff removal requires a strict UUID", async (t) => {
  const root = temporaryDirectory("cinba-update-handoff-remove-id-", t);
  const current = desktopHandoff(root);
  await writeUpdateHandoff(root, current, { now });
  await assert.rejects(() => removeUpdateHandoff(root, "../handoff", { now }), /id must be a UUID/);
  assert.deepEqual(JSON.parse(await readFile(updateHandoffPath(root), "utf8")), current);
});

test("the next lease reaps an expired claimed record left by a crashed helper", async (t) => {
  const root = temporaryDirectory("cinba-update-claimed-expired-", t);
  const stale = desktopHandoff(root);
  const retryAt = new Date(now.getTime() + UPDATE_HANDOFF_TTL_MS + 1);
  await writeUpdateHandoff(root, stale, { now });
  const claimed = await claimUpdateHandoff(root, { now });
...
test("claimed scan retains a fresh diagnostic record without blocking new handoff state", async (t) => {
  const root = temporaryDirectory("cinba-update-claimed-fresh-", t);
...
test("a new handoff never reuses an existing claimed record id", async (t) => {
  const root = temporaryDirectory("cinba-update-claimed-same-id-", t);
...
test("claimed scan removes mismatched and malformed direct files without touching unrelated files", async (t) => {
  const root = temporaryDirectory("cinba-update-claimed-invalid-", t);
...
test("claimed scan unlinks a strict-name symlink without following its target", async (t) => {
  const root = temporaryDirectory("cinba-update-claimed-link-", t);
  const outside = temporaryDirectory("cinba-update-claimed-outside-", t);
...
test("claimed scan recovers a symlinked handoff directory without following it", async (t) => {
  const root = temporaryDirectory("cinba-update-claimed-dir-link-", t);
  const outside = temporaryDirectory("cinba-update-claimed-dir-outside-", t);
