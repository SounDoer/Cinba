...
test("a remote Core profile survives restarting the Desktop store", () => {
  const directory = mkdtempSync(join(tmpdir(), "cinba-desktop-profiles-"));
...
test("the Desktop reopens the last selected Core profile", () => {
  const directory = mkdtempSync(join(tmpdir(), "cinba-desktop-profiles-"));
  const path = join(directory, "desktop.json");
  try {
    const store = createCoreProfileStore(path, {
...
test("the Desktop refuses duplicate Core origins", () => {
  const directory = mkdtempSync(join(tmpdir(), "cinba-desktop-profiles-"));
  const path = join(directory, "desktop.json");
  try {
...
test("a user can edit a saved remote Core profile", () => {
  const directory = mkdtempSync(join(tmpdir(), "cinba-desktop-profiles-"));
  const path = join(directory, "desktop.json");
  try {
...
test("a user can remove a remote Core that is not selected", () => {
  const directory = mkdtempSync(join(tmpdir(), "cinba-desktop-profiles-"));
  const path = join(directory, "desktop.json");
  try {
    const store = createCoreProfileStore(path, { createId: () => "profile-1" });
...
test("the selected Core profile cannot be removed", () => {
  const directory = mkdtempSync(join(tmpdir(), "cinba-desktop-profiles-"));
  const path = join(directory, "desktop.json");
  try {
    const store = createCoreProfileStore(path, { createId: () => "profile-1" });
...
test("a damaged Desktop profile file leaves the Local Core available", () => {
  const directory = mkdtempSync(join(tmpdir(), "cinba-desktop-profiles-"));
  const path = join(directory, "desktop.json");
  try {
...
test("a damaged Desktop profile file is not overwritten by a normal edit", () => {
  const directory = mkdtempSync(join(tmpdir(), "cinba-desktop-profiles-"));
  const path = join(directory, "desktop.json");
  try {
    writeFileSync(path, "{broken", "utf8");
...
test("opening the Local Core cannot overwrite a damaged profile file", () => {
  const directory = mkdtempSync(join(tmpdir(), "cinba-desktop-profiles-"));
  const path = join(directory, "desktop.json");
  try {
    writeFileSync(path, "{broken", "utf8");
...
test("one invalid remote Core does not hide the rest of the profile list", () => {
  const directory = mkdtempSync(join(tmpdir(), "cinba-desktop-profiles-"));
  const path = join(directory, "desktop.json");
  try {
    writeFileSync(
...
test("an invalid Desktop settings document is preserved as unreadable", () => {
  const directory = mkdtempSync(join(tmpdir(), "cinba-desktop-profiles-"));
  const path = join(directory, "desktop.json");
  try {
...
test("an unreadable Desktop settings file can be recovered without losing its original", () => {
  const directory = mkdtempSync(join(tmpdir(), "cinba-desktop-profiles-"));
  const path = join(directory, "desktop.json");
  try {
    writeFileSync(path, "{broken", "utf8");
...
test("a missing last-selected profile falls back to Local", () => {
  const directory = mkdtempSync(join(tmpdir(), "cinba-desktop-profiles-"));
  const path = join(directory, "desktop.json");
  try {
    writeFileSync(
...
test("Desktop surfaces refresh after a profile is saved", () => {
  const directory = mkdtempSync(join(tmpdir(), "cinba-desktop-profiles-"));
  const path = join(directory, "desktop.json");
  try {
    const store = createCoreProfileStore(path, { createId: () => "vps" });
...
test("editing a Core cannot duplicate another saved origin", () => {
  const directory = mkdtempSync(join(tmpdir(), "cinba-desktop-profiles-"));
  const path = join(directory, "desktop.json");
  let nextId = 0;
  try {
...
test("a failed profile save leaves the last complete file readable", () => {
  const directory = mkdtempSync(join(tmpdir(), "cinba-desktop-profiles-"));
  const path = join(directory, "desktop.json");
  try {
...
test("failed updates, removals, and selections roll back live profile state", () => {
  const directory = mkdtempSync(join(tmpdir(), "cinba-desktop-profiles-"));
  const path = join(directory, "desktop.json");
  try {
    createCoreProfileStore(path, { createId: () => "vps" }).add({
...
test("Desktop surfaces refresh after profiles are edited or removed", () => {
  const directory = mkdtempSync(join(tmpdir(), "cinba-desktop-profiles-"));
  const path = join(directory, "desktop.json");
  try {
    const store = createCoreProfileStore(path, { createId: () => "vps" });
