...
test("reuses a Core that is already healthy", async (t) => {
  const home = temporaryDirectory("cinba-manager-", t);
  const config = createTestCoreConfig(home);
  let spawns = 0;
  let locks = 0;
...
test("starts one managed Core and records its runtime", async (t) => {
  const home = temporaryDirectory("cinba-manager-", t);
  const config = createTestCoreConfig(home);
  let probes = 0;
  let releases = 0;
...
test("an existing managed persistent Core is returned to on-demand", async (t) => {
  const home = temporaryDirectory("cinba-manager-", t);
  const config = createTestCoreConfig(home);
  let running = false;
...
test("normalizing lifetime never starts a stopped Core", async (t) => {
  const home = temporaryDirectory("cinba-manager-", t);
  const config = createTestCoreConfig(home);
...
test("a safe managed Core from an old revision is restarted", async (t) => {
  const home = temporaryDirectory("cinba-manager-", t);
  const config = createTestCoreConfig(home);
  let running = false;
...
test("a busy managed Core from an old revision is not stopped", async (t) => {
  const home = temporaryDirectory("cinba-manager-", t);
  const config = createTestCoreConfig(home);
  let running = false;
  let stopRequests = 0;
...
test("an external Core from another revision is never stopped or replaced", async (t) => {
  const home = temporaryDirectory("cinba-manager-", t);
  const config = createTestCoreConfig(home);
  let stops = 0;
  let spawns = 0;
...
test("a draining managed Core finishes before one replacement is started", async (t) => {
  const home = temporaryDirectory("cinba-manager-", t);
  const config = createTestCoreConfig(home);
  let running = false;
  let draining = false;
...
test("rechecks health after taking the lock", async (t) => {
  const home = temporaryDirectory("cinba-manager-", t);
  const config = createTestCoreConfig(home);
  let probes = 0;
  let spawns = 0;
...
test("reuses an on-demand Core discovered after taking the start lock", async (t) => {
  const home = temporaryDirectory("cinba-manager-", t);
  const config = createTestCoreConfig(home);
  let running = false;
  await ensureLocalCore({
...
test("concurrent callers produce only one Core process", async (t) => {
  const home = temporaryDirectory("cinba-manager-", t);
  const config = createTestCoreConfig(home);
  let healthy = false;
  let spawns = 0;
...
test("a managed Core is asked to stop gracefully", async (t) => {
  const home = temporaryDirectory("cinba-manager-", t);
  const config = createTestCoreConfig(home);
  let running = false;
  let stopRequests = 0;
...
test("an external Core cannot be stopped through stale or missing local records", async (t) => {
  const home = temporaryDirectory("cinba-manager-", t);
  const config = createTestCoreConfig(home);
