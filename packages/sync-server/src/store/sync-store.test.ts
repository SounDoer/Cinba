...
function fixture(t: TestContext): { directory: string } {
  return { directory: temporaryDirectory("cinba-sync-store-", t) };
}
...
test("first initialization and restart retain one stable server identity", (t) => {
...
test("an interrupted first initialization leaves no half-created key or state", (t) => {
  const files = fixture(t);
...
test("concurrent mutations are serialized with continuous Sync revisions", async (t) => {
  const files = fixture(t);
  const store = createSyncStore(files.directory);
  assert.deepEqual(
    await Promise.all([
...
test("Settings, Credential, and rollback mutations use their distinct revision rules", async (t) => {
  const files = fixture(t);
  let tick = 0;
  const store = createSyncStore(files.directory, {
    now: () => {
...
test("a stale Settings revision is rejected without changing current data", async (t) => {
  const files = fixture(t);
  const store = createSyncStore(files.directory);
...
test("credentials persist only as ciphertext and decrypt after restart", async (t) => {
  const files = fixture(t);
  const store = createSyncStore(files.directory);
...
test("an interrupted atomic write retains the old complete file and removes its temporary file", async (t) => {
  const files = fixture(t);
  let interrupt = false;
  const store = createSyncStore(files.directory, {
    atomicWrite: {
...
test("copying state.json without credential-key cannot reveal or mutate credentials", async (t) => {
...
test("corrupt, unknown-version, and missing-key stores fail closed without overwrite", async (t) => {
  for (const damage of ["corrupt", "unknown-version", "missing-key"] as const) {
    ...
    await assert.rejects(store.setCredential("deepseek", "replacement"), SyncStoreUnavailableError);
    assert.equal(readFileSync(path, "utf8"), original);
  }
});

test("non-monotonic history revisions make a store read-only", async (t) => {
  const files = fixture(t);
  const store = createSyncStore(files.directory);
  await store.updateSettings(0, settings("current"));
  const state = readState(files.directory);
...
test("a credential authentication failure makes the whole store read-only", async (t) => {
  const files = fixture(t);
  const store = createSyncStore(files.directory);
