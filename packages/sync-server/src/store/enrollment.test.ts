...
test("pending enrollment transitions to rejected or expired and stops polling", async () => {
  const directory = mkdtempSync(join(tmpdir(), "cinba-enrollment-"));
...
test("approval delivers one Core credential once and stores only its salted hash", async () => {
  const directory = mkdtempSync(join(tmpdir(), "cinba-enrollment-"));
  try {
...
test("Core credentials are isolated, revocation is immediate, and Local cores never get secrets", async () => {
  const directory = mkdtempSync(join(tmpdir(), "cinba-enrollment-"));
  try {
    const store = createSyncStore(directory);
...
test("capability reports produce an explicit partial-support model catalog", async () => {
  const directory = mkdtempSync(join(tmpdir(), "cinba-enrollment-"));
  try {
    const store = createSyncStore(directory);
