...
function fixture() {
  const directory = mkdtempSync(join(tmpdir(), "cinba-admin-auth-"));
...
test("Setup Code succeeds once, is removed, and cannot revive after restart", async () => {
  const files = fixture();
  try {
...
test("passwords never land in plaintext and repeated setup uses independent salts", async () => {
  const files = fixture();
  try {
...
test("login, logout, expiry, and process-local sessions have distinct states", async () => {
  const directory = mkdtempSync(join(tmpdir(), "cinba-admin-auth-"));
...
test("management writes require matching Origin, Cookie, and CSRF without replay", async () => {
  const files = fixture();
  try {
...
test("local password reset invalidates sessions and the old password", async () => {
  const files = fixture();
  try {
    const setup = await files.service.setup(
      files.store.localSetupCode()!,
    ...
    assert.equal(
      (await files.service.setup(resetCode, "replacement-password", security())).ok,
      true,
    );
...
test("failed setup and login attempts are rate limited without retaining input values", async () => {
  const directory = mkdtempSync(join(tmpdir(), "cinba-admin-auth-"));
  try {
    const store = createSyncStore(directory);
...
test("failed login attempts use the same bounded in-memory rate policy", async () => {
  const directory = mkdtempSync(join(tmpdir(), "cinba-admin-auth-"));
  try {
    const store = createSyncStore(directory);
    const service = new AdministratorAuthService({
...
test("cookie security is strict in deployment and explicitly relaxed only for loopback", async () => {
  const secureFiles = fixture();
  const developmentDirectory = mkdtempSync(join(tmpdir(), "cinba-admin-auth-"));
  try {
    ...
    const result = await development.setup(
      developmentStore.localSetupCode()!,
      "a-strong-password",
      { origin: "http://127.0.0.1:9000", clientKey: "local" },
    );
    assert.equal(result.ok, true);
    if (result.ok) {
    ...
    secureFiles.close();
    rmSync(developmentDirectory, { recursive: true, force: true });
  }
});
